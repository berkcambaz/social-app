...
async function login(req: Request, res: Response, next: NextFunction) {
  ...
  if (data.usertag === undefined) return res.status(404);
  if (data.password === undefined) return res.status(404);

  // Usertag should be between 3 - 16 characters
  if (data.usertag.length < 3 || data.usertag.length > 16) return res.status(404);

  // Check if password is not shorter than 8 characters
  if (data.password.length < 8) return res.status(404);
  ...
  if (result.length === 0 || err) return res.status(404);
...
async function signup(req: Request, res: Response, next: NextFunction) {
  ...
  // Check if data is undefined
  if (data.usertag === undefined) return res.status(404);
  if (data.email === undefined) return res.status(404);
  if (data.password === undefined) return res.status(404);
  ...
  if (!validate(data.email)) return res.status(404);
