...
const onInit = async () => {
  ...
  if (user.value !== null)
    await users.fetchUserFollowings(user.value.id, "newer", true);
}

const onTop = async () => {
  if (user.value !== null) await users.fetchUserFollowings(user.value.id, "newer");
}

const onBottom = async () => {
  if (user.value !== null) await users.fetchUserFollowings(user.value.id, "older");
}
</script>
